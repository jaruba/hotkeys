...
Keymap.prototype.trigger = function(hotkey) {
    if (typeof hotkey === 'string') hotkey = parseKeyCombo(hotkey);
    
    var forChar = this._map[hotkey.ch] || (this._map[hotkey.ch] = []);

	forChar.some(function(el,ij) {
		if (JSON.stringify(el.hotkey) == JSON.stringify(hotkey)) {
			el.action();
			return true;
		}
	});
